...
vec2 VectorRotateTime(vec2 v, float speed)
{
    ...
    if ((localTime >= 0.0) && (localTime < 0.25)) rotation = 0.0;
    else if ((localTime >= 0.25) && (localTime < 0.50)) rotation = PI/4.0*sin(2.0*PI*localTime - PI/2.0);
    else if ((localTime >= 0.50) && (localTime < 0.75)) rotation = PI*0.25;
    else if ((localTime >= 0.75) && (localTime < 1.00)) rotation = PI/4.0*sin(2.0*PI*localTime);
